...
def run_epoch(model, loader, optimizer, cfg, device, train: bool) -> float:
    ...
    desc = "train_dit" if train else "val_dit"
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Train tiny DiT-style spectrogram Transformer denoiser.")
    ...
    for epoch in range(1, int(cfg["epochs"]) + 1):
        ...
        torch.save(checkpoint, save_dir / "last_dit.pt")
        if val_loss < best_loss:
            best_loss = val_loss
            torch.save(checkpoint, save_dir / "best_dit.pt")
    log_df = pd.DataFrame(rows)
    log_path = metrics_dir / "train_dit_log.csv"
    log_df.to_csv(log_path, index=False)
    plot_loss_curve(log_df, figures_dir / "dit_loss_curve.png")
    print(f"Wrote DiT-style train log: {log_path}")
    print(f"Wrote best DiT-style checkpoint: {save_dir / 'best_dit.pt'}")
