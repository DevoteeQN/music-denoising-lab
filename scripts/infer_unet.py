...
def denoise_full_audio(model, audio: np.ndarray, cfg: dict, device: torch.device) -> np.ndarray:
    ...
    for start in range(0, len(audio), segment_samples):
        ...
        denoised = denoise_segment(model, chunk, cfg, device)[:original_len]
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Run U-Net denoising on an audio file.")
    ...
    print(f"Wrote U-Net output: {args.output}")
    print(f"Wrote U-Net MP3: {Path(args.output).with_suffix('.mp3')}")
