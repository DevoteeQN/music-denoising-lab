...
from scripts.infer_unet import denoise_full_audio, load_model
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Evaluate baseline and U-Net on paired synthetic test set.")
    ...
    parser.add_argument("--output_csv", default="outputs/metrics/test_metrics.csv")
        ...
        [
            {
                ...
                "mean_snr_unet": float(per_sample["snr_unet"].mean()),
                "mean_snri_baseline": float(per_sample["snri_baseline"].mean()),
                "mean_snri_unet": float(per_sample["snri_unet"].mean()),
            }
        ]
    )
    ...
    print(f"Wrote summary metrics: {output_csv}")
